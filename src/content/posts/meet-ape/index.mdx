...
description: "We shipped a greenfield project in three weeks by building the system around the AI models, not the prompts. I call it APE: Agentic Product Engineering."
...
The fastest way I've found to sharpen a fuzzy spec is to be interrogated about it. [Mat Pocock](https://www.aihero.dev/)'s "grill me" skill does exactly that — the agent grills you on assumptions, edge cases, and contradictions until the spec is something a fleet of agents can act on without guessing. Use it before you spawn anything.
...
4. **Try one agent team on one phase.** Not your whole project. Pick a greenfield slice with clean file boundaries. Spawn a few agents with explicit file ownership and a reviewer. Watch what breaks. Fix the harness, not the prompt. ([Here's a real parallel-review session](/blog/ai-coding-workflow-what-worked) where the reviewer agent caught bugs the tests missed.)
