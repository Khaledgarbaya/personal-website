...
Consider: accuracy, completeness, practicality, and insight.`;
  ...
  private determineWinner(votes: Map<string, number>): { agentId: string; response: string; votes: number } {
    ...
    const winnerAgent = this.agents.get(winnerId);
    
    return {
      agentId: winnerId,
      response: '', // We'll get this from our stored responses
      votes: maxVotes,
    };
  }
...
private async updatePerformance(winnerId: string, queryId: string) {
...
Let's add proper evaluation using Mastra's built-in scorers:
