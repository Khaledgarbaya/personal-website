...
description: "AI-assisted coding patterns that actually work, from months of experiments: two case studies — delegated testing and parallel review with Claude Code."
...
I've been experimenting with AI-assisted coding for a while now, and most of it has been... fine. Helpful for boilerplate, decent for explaining code, occasionally useful for debugging. But last week, something clicked. I found a workflow pattern that felt genuinely different: I'd finally figured out how to work *with* the AI instead of just *using* it.
...
<TerminalSimulation
  ...
  lines={[
    { type: 'speaker', label: 'Me', content: '"Test these 6 things, server is running, I\'ll provide magic links"', color: 'hsl(43 96% 56%)' },
    { type: 'arrow', content: '↓' },
    { type: 'action', label: 'Claude', content: '[Creates todo list, executes tests, takes screenshots, reports progress]' },
    { type: 'arrow', content: '↓' },
    { type: 'speaker', label: 'Me', content: '[Provides magic link when prompted]', color: 'hsl(43 96% 56%)' },
    { type: 'arrow', content: '↓' },
    { type: 'action', label: 'Claude', content: '[Continues autonomously]' },
    { type: 'arrow', content: '↓' },
    { type: 'speaker', label: 'Me', content: '"Update the PR with results"', color: 'hsl(43 96% 56%)' },
    { type: 'arrow', content: '↓' },
    { type: 'action', label: 'Claude', content: '[Updates PR via gh CLI]' },
    { type: 'arrow', content: '↓' },
    { type: 'speaker', label: 'Me', content: '"PR merged, switch to develop"', color: 'hsl(43 96% 56%)' },
  ]}
/>
...
Browser automation, bash commands, and GitHub CLI worked together. Test in browser, capture results, update PR via CLI. I didn't have to context-switch between tools or copy-paste results between windows.

### Trust but Verify

Screenshots provided proof of each step. I could skim them or ignore them entirely. My choice. But they were there if I needed them, which meant I could trust the process without babysitting it.
...
This is the part that stuck with me. In a traditional testing session, I would have:
...
The magic isn't in any single capability. It's in the composition. Browser automation alone isn't new. Todo lists aren't new. Git integration isn't new. But combining them with an AI that can adapt to context and figure out the details? That's where the leverage comes from.

Testing isn't the only place this works. Here's another workflow that surprised me more.
...
<TerminalSimulation
  ...
  lineDelay={350}
  lines={[
    { type: 'numbered', label: '1.', content: 'PARALLEL EXPLORATION', color: 'hsl(191 91% 55%)' },
    { type: 'indent', content: '└── Multiple agents analyzing different domains simultaneously' },
    { type: 'numbered', label: '2.', content: 'DOCUMENTATION LOOKUP', color: 'hsl(191 91% 55%)' },
    { type: 'indent', content: '└── Context7 MCP for current library best practices' },
    { type: 'numbered', label: '3.', content: 'PLAN DESIGN', color: 'hsl(191 91% 55%)' },
    { type: 'indent', content: '└── Synthesizing findings into improvements' },
    { type: 'numbered', label: '4.', content: 'TARGETED AUDIT', color: 'hsl(191 91% 55%)' },
    { type: 'indent', content: '└── Deep dive on specific concerns (auth consistency)' },
    { type: 'numbered', label: '5.', content: 'STRUCTURED OUTPUT', color: 'hsl(191 91% 55%)' },
    { type: 'indent', content: '└── Prioritized, actionable improvements' },
  ]}
/>

### Three Agents in Parallel, Not One in Sequence
...
The output wasn't just a list of problems. It was a structured plan: before/after code examples, prioritized by impact, with specific file paths and line numbers.
...
I think we're still early in figuring out how to work effectively with AI coding assistants. The instinct is to use them like autocomplete on steroids, but the real leverage comes from treating them like a team of junior developers who can work in parallel, follow specific instructions, and report back structured findings. That's the same bet behind [APE — Agentic Product Engineering](/blog/meet-ape), the system I use to run this with a real team — and when several engineers do it against one codebase, [new problems show up at team scale](/blog/ai-engineering-workflows-team-scale).

The tooling will keep changing. The discipline — tight scope, parallelism, checkpoints, captured context — is what carries over.
